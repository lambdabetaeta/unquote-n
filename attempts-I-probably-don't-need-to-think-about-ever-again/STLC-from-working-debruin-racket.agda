...
mutual
  ctxType : Ctx → Set
  ctxType ∅ = ⊤
  ctxType (Γ , T) = Σ (ctxType Γ) (λ γ → Maybe (GExp Γ T))
...
unquote-n : ∀{Γ T} → Exp Γ T → (γ : ctxType Γ) → GExp Γ T
unquote-n (var icx) γ ren count = {!   !}
unquote-n (lambda e) γ ren none = lambda (unquote-n e (γ , nothing) (liftRen ren) none)
unquote-n (lambda e) γ ren (one count)
  -- = λ g → unquote-n e (γ , just {! g   !} ) {! ren  !} count
  = λ g → unquote-n e (γ , just {! g  !} ) {! ren  !} count
  -- TODO: ren SHOULDN'T fit in the hole, should be "w-1" , not "w".
  -- So maybe something is wrong about typing of unquote-n (maybe reduceCtx?)
  -- need to be able to have this correspond to racket!
  -- maybe GExp needs to be able to refer to reduceCtx?
unquote-n (app e₁ e₂) γ ren count
  = unquote-n e₁ γ ren (one count) (λ ren₁ count → unquote-n e₂ γ (ren ∘ ren₁) count)
unquote-n true γ ren count = {!   !}
unquote-n false γ ren count = {!   !}
unquote-n (if e e₁ e₂) γ ren count = {!   !}
