...
mutual
  ...
  PUExp : Ctx → (T : Type) → ArgCount T → Set
  PUExp Γ T none = Nf Γ T
  PUExp Γ (A ⇒ B) (one count) = (GExp Γ A) → PUExp Γ B count

  -- Exp that can be in a weaker context AND partially unquoted to any degree
  GExp : Ctx → Type → Set
  GExp Γ T = ∀{Γ'} → Ren Γ' Γ → (count : ArgCount T) → PUExp Γ' T count
...
nApp : ∀{Γ T} → (count : ArgCount T) → Ne Γ T → PUExp Γ T count
nApp none e = ne e
...
unquote-n : ∀{Γ Δ T} → Exp Γ T → Sub Δ Γ → (count : ArgCount T) → PUExp Δ T count
...
e5 : Exp ∅ (nat ⇒ (nat ⇒ nat)) -- λ x. λ y. x
e5 = lambda (lambda (var (next same)))

test5 : normalize (app (app e5 (nat 1)) (nat 2)) ≡ nat 1
test5 = refl
