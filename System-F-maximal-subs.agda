...
nApp sub (one count) e = λ x → nApp sub count (app e (x idRen none))
  ...
  -- (append1sub idSub X) ∘ (liftTSub sub) = append1sub sub X
...
nApp sub (One X count) e = nApp (append1sub sub X) count (TApp e X)
  ...
  -- idSub ∘ sub ≡ sub
nApp {suc n} sub (cumu count) e = nApp idSub count {!   !}
-- nApp sub (cumu count) (cumu e) = nApp idSub count e
