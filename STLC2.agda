...
mutual
  ...
  inputs none Γ = ⊤
  inputs (one {A} count) Γ
    = ((count : ArgCount A) → (inputs count Γ) → Exp Γ (output count)) × (inputs count Γ)
...
mutual
  ctxType : Ctx → Set
  ctxType ∅ = ⊤
  ctxType (Γ , T) = Σ (ctxType Γ)
    (λ γ → Maybe ((count : ArgCount T) → inputs count (reduceCtx Γ γ) → Exp (reduceCtx Γ γ) (output count)))
...
nVarApp : ∀{Γ T} → (count : ArgCount T)-- → ToType count (Γ , T)
  → inputs count (Γ , T) → Exp (Γ , T) (output count)
nVarApp none ins = var same
nVarApp (one none) (e , ins) = app (nVarApp none ins) (e none tt)
nVarApp (one (one none)) (e₁ , e₂ , tt)
  = app (nVarApp (one none) (e₁ ,  tt) ) (e₂ none tt)
-- TODO: TODO: TODO: WHY do e₁ and e₂ switch places?!?!?!?
nVarApp = {!   !}
-- nVarApp (one count) (e , ins) = app {! nVarApp count ?  !} {!   !}
{-
