...
TCtx' = {!   !}
data Ctx' : Set₁ where
  ctx : (TCtx' → Type → String → Set) → Ctx'

∅ : Ctx
∅ T name = ⊥

Weakening : Ctx → Ctx → Set
Weakening Γ₁ Γ₂ = (T : Type) → (s : String) → Γ₁ T s → Γ₂ T s

Equiv : Ctx → Ctx → Set
Equiv Γ₁ Γ₂ = (T : Type) → (s : String) → Γ₁ T s → Γ₂ T s

append : Ctx → Type → String → Ctx
append Γ T name = λ T str → {! if str Data.String.≟ name then ? else ?  !}

InCtx : Ctx → Type → Set
InCtx Γ T = Σ String (λ name → Γ T name)

-- Macro : (Γ : Ctx) → (T : Type Γ) → String → InCtx Γ T
-- Or fails if it doesn't exist. Can be written as dependently typed function:
-- (Γ : Ctx) → (T : Type Γ) → (name : String) → (outputType Γ T name)
-- where outputType returns either ⊤ or InCtx Γ T depending on whether it exists or not.
-- So code will only typecheck if it actually existed.
-- PROBLEM : not possible to actually check since can't compute on function.
-- Maybe I really need try tactic?


-- I don't think that this really solves any problems. Only thing is that
-- We can use Equiv instead of ≡.
-- with weakening, ctxts of types don't REALLY need to change.
-- but with sub they do. Think about that.
